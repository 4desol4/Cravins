...
app.use("/api/*", (req, res) => {
  res.status(404).json(apiResponse(false, "API endpoint not found"));
});
...
app.use((error, req, res, next) => {
  ...
  if (error.name === "ValidationError") {
    ...
    return res
      .status(400)
      .json(apiResponse(false, "Validation failed", null, { errors }));
  }

  // Mongoose duplicate key error
  if (error.code === 11000) {
    const field = Object.keys(error.keyValue)[0];
    return res.status(400).json(apiResponse(false, `${field} already exists`));
  }
  ...
  if (error.name === "TokenExpiredError") {
    return res.status(401).json(apiResponse(false, "Token has expired"));
  }
  ...
  if (error instanceof multer.MulterError) {
    if (error.code === "LIMIT_FILE_SIZE") {
      return res
        .status(400)
        .json(apiResponse(false, ERROR_MESSAGES.FILE_TOO_LARGE));
    }
  ...
  // Prisma errors
  if (error.code === "P2002") {
    return res.status(400).json(apiResponse(false, "Duplicate entry detected"));
  }
